...
namespace SlidingCubes {
    ...
    void SlidingCubesGlBlock::fireSelectedTrigger() {
        ...
        const Cell3DPosition pos = lattice->worldToGridPosition(getPosition());
        SlidingCubesBlock *sc = static_cast<SlidingCubesBlock *>(lattice->getBlock(pos));
        if (sc && sc->blockCode) sc->blockCode->onBlockSelected();
    }
}
