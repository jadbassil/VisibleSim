...
def plot_single_results(csv_file):
    ...
    if 'num_messages' in df.columns:
        ax1.plot(range(len(x_data)), df['num_messages'], color='steelblue', linewidth=2.5, marker='o', markersize=8)
        ax1.set_xlabel(x_label, fontsize=12)
        ax1.set_ylabel('Number of Messages', fontsize=12)
        ax1.set_title('Number of Messages per Configuration', fontsize=14, fontweight='bold')
    ...
    if 'simulator_time_us' in df.columns:
        ...
        ax2.plot(range(len(x_data)), simulator_time_ms, color='coral', linewidth=2.5, marker='o', markersize=8)
        ax2.set_xlabel(x_label, fontsize=12)
        ax2.set_ylabel('Time (ms)', fontsize=12)
        ax2.set_title('Time per Configuration', fontsize=14, fontweight='bold')
...
def plot_comparison(csv_files):
    ...
    for config_name, df in dfs.items():
        ...
        # Plot 1: Number of Messages
        if 'num_messages' in df.columns:
            ax1.plot(x_data, df['num_messages'].values, color='steelblue', linewidth=2.5, marker='o', markersize=8)
            ax1.set_xlabel(x_label, fontsize=12)
            ax1.set_ylabel('Number of Messages', fontsize=12)
            ax1.set_title('Number of Messages', fontsize=14, fontweight='bold')
        ...
        if 'simulator_time_us' in df.columns:
            ...
            ax2.plot(x_data, simulator_time_ms, color='coral', linewidth=2.5, marker='o', markersize=8)
            ax2.set_xlabel(x_label, fontsize=12)
            ax2.set_ylabel('Time (ms)', fontsize=12)
            ax2.set_title('Time', fontsize=14, fontweight='bold')
...
def main():
    ...
    if len(args.csv_files) > 1:
        ...
        plot_comparison(args.csv_files)
    elif len(args.csv_files) == 1:
        # Single file
        plot_single_results(args.csv_files[0])
    else:
        ...
        if len(existing_files) > 1:
            ...
            plot_comparison(existing_files)
        else:
            # Fallback to single plot
            plot_single_results('simulation_results_cube.csv')
