    ...
    if(isTop()) {
        ...
        if(lattice->cellHasBlock(module->position.offsetY(-1))) {
            sendMessage("SetL msg", new MessageOf<int>(SETL_MSG_ID, l),
                        module->getInterface(lattice->getCellInDirection(module->position, FCCLattice::Direction::C7South)), 100, 0);
        }
...
void ShapeRecognitionBlockCode::handleSetLMessage(std::shared_ptr<Message> _msg,
    ...
    if(lattice->cellHasBlock(module->position.offsetY(-1))) {
        ...
        if(lattice->cellHasBlock(module->position.offsetX(1))) {
            nbWaitedCheckLR++;
            sendMessage("CheckLR<right> msg", new Message(CHECKLR_MSG_ID), module->getInterface(module->position.offsetX(1)), 100, 0);
        }
        if(lattice->cellHasBlock(module->position.offsetX(-1))) {
            nbWaitedCheckLR++;
            sendMessage("CheckLR<left> msg", new Message(CHECKLR_MSG_ID), module->getInterface(module->position.offsetX(-1)), 100, 0);
        }
...
void ShapeRecognitionBlockCode::handleSendLRMessage(std::shared_ptr<Message> _msg, P2PNetworkInterface *sender) { {
    ...
    if(nbWaitedCheckLR == 0) {
        ...
        if(l != rightL and l != leftL) {
            ...
            sendMessageToAllNeighbors("BroadcastRect msg", new MessageOf<Rectangle>(BROADCASTRECT_MSG_ID, myRectangle), 100, 0, 0);
        }
        if(l == leftL and rightL != l) {
            sendMessage("GetW message", new MessageOf<int>(GETW_MSG_ID, 1), module->getInterface(module->position.offsetX(-1)), 100, 0);
        }
    ...
    if(sender == module->getInterface(module->position.offsetX(1))) { // coming from right
        if(leftL != l) {
          /*  myRectangle = Rectangle(module->position, l, recvW);*/
            sendMessage("SetW message", new MessageOf<int>(SETW_MSG_ID, recvW), module->getInterface(module->position.offsetX(1)), 100, 0);

        } else {
            sendMessage("GetW message", new MessageOf<int>(GETW_MSG_ID, recvW), module->getInterface(module->position.offsetX(-1)), 100, 0);
        }
    } else {
        if(rightL != l) {
/*
...
void ShapeRecognitionBlockCode::handleBroadcastRectWMessage(std::shared_ptr<Message> _msg,
                                                            P2PNetworkInterface *sender) {
    MessageOf<Rectangle>* msg = static_cast<MessageOf<Rectangle>*>(_msg.get());
    Rectangle recvRectangle = *msg->getData();
    if(find(currentShape.begin(), currentShape.end(), recvRectangle) != currentShape.end() or isInshape(recvRectangle.base)) return;
    currentShape.insert(recvRectangle);
    sendMessageToAllNeighbors("BroacastRect msg", new MessageOf<Rectangle>(BROADCASTRECT_MSG_ID, recvRectangle), 100, 0, 1, sender);
}
