...
void CoordinateBackMessage::handle(BaseSimulator::BlockCode *bc) {
    ...
    if (rbc.module->position == coordinatorPosition) {
        ...
        if (rbc.operation->isFill() and rbc.operation->getDirection() == Direction::LEFT and rbc.transferCount >= 8 and
            steps == 0) {
            return;
        }
    ...
    if (rbc.operation) {
        if (/*rbc.operation->isTransfer() and*/ rbc.operation->getDirection() == Direction::BACK and
            ...
            if (rbc.relativePos() == Cell3DPosition(1, 1, 2) and
                (srcPos == rbc.seedPosition + Cell3DPosition(-2, 0, 2) or
                 srcPos == rbc.seedPosition + Cell3DPosition(-2, 1, 2))) {
        ...
        if(rbc.isCoordinator and rbc.getPreviousOpDir() == Direction::LEFT and srcPos.pt[2] > rbc.seedPosition.pt[2] + 2
            and rbc.module->getInterface(rbc.module->position.offsetY(-1))->isConnected()) {
            getScheduler()->trace("light turned orange7\n", rbc.module->blockId, ORANGE);
            if (not rbc.awaitingSources.empty()) rbc.setGreenLight(true);
            rbc.awaitingSources.insert(srcPos);
            //rbc.module->setColor(DARKORANGE);
            return;
