...
void GoMessage::handle(BaseSimulator::BlockCode *bc) {
    ...
    if(rbc->parentPosition == Cell3DPosition(-1,-1,-1) and rbc->module->blockId != RePoStBlockCode::GC->blockId) {
        rbc->parentPosition = fromMMPosition;
        // distance = data->distance + 1;
        rbc->isPotentialSource() ? rbc->distance = distance + 1: 
    ...
    } else if( (not rbc->isPotentialSource() and distance < rbc->distance) or (rbc->isPotentialSource() and distance + 1 < rbc->distance)) {
        Cell3DPosition toSeedPosition = rbc->getSeedPositionFromMMPosition(rbc->parentPosition);
...
void BackMessage::handle(BaseSimulator::BlockCode *bc) {
    ...
    if(rbc->nbWaitedAnswers == 0) {
        if(rbc->parentPosition == Cell3DPosition(-1,-1,-1) and rbc->module->position == RePoStBlockCode::GC->position) {
