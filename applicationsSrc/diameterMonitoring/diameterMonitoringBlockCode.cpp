...
void DiameterMonitoringBlockCode::handleFarthestMessage(std::shared_ptr<Message> _msg,
    ...
    if(interfaceToFarthest != nullptr) {
        ...
        if(du != -1 and dv == -1) setColor(GREEN);
        else if (dv != -1) setColor(YELLOW);
        else setColor(BLUE);
        ...
        if(round <= 3){
            ...
            sendMessageToAllNeighbors("NOTIFYDIAMETER",
                                      new MessageOf<int>(NOTIFYDIAMETER_MSG_ID,D),100,200, 0);
        }
    }
}
...
void DiameterMonitoringBlockCode::handleNotifyDiameterMessage(std::shared_ptr<Message> _msg,
    ...
    if(D == -1) {
        ...
        sendMessageToAllNeighbors("NOTIFYDIAMETER",
                                  new MessageOf<int>(NOTIFYDIAMETER_MSG_ID,D),100,200, 1,sender);
        setColor(WHITE);
        if (dv == -1) {
            du = 0; setColor(GREEN);
        }
